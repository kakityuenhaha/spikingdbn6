...
#for offset in range(0, 1000, num_test):
    ...
    #k_size=in_size-out_size+1
        ...
        #print output_size
            ...
            if len(conn_list_exci)>0:
                connections['e2e'] = p.Projection(pop_in,pop_out,p.FromListConnector(conn_list_exci),label='excitatory')
            if len(conn_list_inhi)>0:
                connections['i2i'] = p.Projection(pop_in,pop_out,p.FromListConnector(conn_list_inhi),label='inhibitory')
